...
function getLine(node) { // TODO: more robust way of getting line numbers
  const ast = inputs.find(function (x) {
    return x.file === current_file
  }).ast;
...
TObject.prototype.getMember = function (name, optional) {
  let t = this.properties.get(name);
  if (!t) {
    t = {
      optional: !!optional,
...
TObject.prototype.toString = function () {
  ...
  this.calls.forEach(function (call) {
    prtys.push('(' + call.parameters.map(function (p) {
      return p.name + ':' + p.type
    }).join(',') + ') => ' + call.returnType)
  });
  return '{' + prtys.join(', ') + '}'
};

// The any type (only one instance of this)
const TAny = new TBuiltin('any');
...
function addModuleMember(member, moduleObject, qname) {
  current_node = member;
  const topLevel = qname === '';
  if (member instanceof TypeScript.FunctionDeclaration) {
    const obj = moduleObject.getMember(member.name.text());
    ...
    if (member.isEnum()) { // enums are ModuleDeclarations in the AST, but they are semantically quite different
      const enumObj = moduleObject.getModule(name);
      enumObj.meta.isEnum = true;
      const enumResult = parseEnum(member, enumObj, qname);
      moduleObject.types.push(name, enumResult.enum)
    } else {
      if (name[0] === '"' || name[0] === "'") { // external module?
        parseExternModule(member)
      } else {
        const submodule = moduleObject.getModule(name);
    ...
    var name = member.name.text();
    const clazzObj = moduleObject.getModule(name);
    const clazz = parseClass(member, clazzObj, qname);
...
function parseExternModule(node) {
  ...
  const name = rawName.substring(1, rawName.length - 1); // remove quotes
  const exportAssignment = node.members.members.find(function (member) {
...
let enum_types = new Map;

function parseEnum(node, objectType, host) {
  current_node = node;
  const qname = qualify(host, node.name.text());
  const enumType = new TEnum(qname);
  const enumMembers = [];
  node.members.members.forEach(function (member) {
...
function parseClass(node, constructorType, host) {
  ...
  const qname = qualify(host, name);
  const instanceType = new TObject(qname, {kind: 'class', origin: current_file});
  instanceType.brand = qname;
  const instanceRef = new TQualifiedReference(qname);

  // put type parameters into scope
  const original_scope = current_scope; // scope to restore before returning
  const static_scope = current_scope; // the scope used by static members (cannot see type parameters)
  const instance_scope = new TTypeParameterScope(current_scope);
  ...
  const selfTypeArgs = typeParams.map(function (tp) {
    return new TTypeParam(tp.name)
  });
  const selfType = selfTypeArgs.length == 0 ? instanceRef : new TGeneric(instanceRef, selfTypeArgs);
  ...
  node.members.members.forEach(function (member) {
    ...
    if (member instanceof TypeScript.FunctionDeclaration) {
      if (member.isConstructor) { // syntax: constructor()..
        ...
        const container = member.isStatic() ? constructorType : instanceType;
...
function parseInterface(node, host) {
  current_node = node;
  const qname = qualify(host, node.name.text());
  const typ = new TObject(qname, {kind: 'interface', origin: current_file});
...
function lookupTypeParameter(scope, name) {
  while (scope !== null) {
    const t = lookupTypeParameterDirect(scope, name);
    if (t)
      return t;
    scope = scope.parent
  }
...
function parseNameList(node) {
  ...
  if (node instanceof TypeScript.Identifier) {
    ...
    const names = parseNameList(node.operand1);
...
function mergePropertyInto(hostPrty, otherPrty) {
  const typ = hostPrty.type;
  const other = otherPrty.type;
...
function renameTypeParametersInType(typ, mapping) {
  if (typ instanceof TTypeParam) {
    const newName = mapping.get(typ.name);
    if (newName)
...
function mergeInto(typ, other) {
  ...
  for (let i = 0; i < typ.typeParameters.length; i++) {
  ...
  other.properties.forEach(function (name, otherPrty) {
    let existing = typ.properties.get(name);
    if (!existing) {
...
function lookupInScope(scope, name, isModule) {
  while (scope !== null) {
    const t = lookupInScopeDirect(scope, name, isModule);
    if (t)
      return t;
    scope = scope.parent
  }
...
function lookupInType(type, name, isModule) {
  const obj = resolveToObject(type);
  if (isModule) {
...
function lookupPrtyInScopeDirect(scope, name) {
  if (scope instanceof TModuleScope) {
    const obj = scope.obj;
    const t = obj.modules.get(name);
    if (t)
      return t;
    const prty = obj.properties.get(name);
    if (prty)
      return prty.type;
    return null;
  }
...
function lookupPrtyInScope(scope, name) {
  while (scope !== null) {
    const t = lookupPrtyInScopeDirect(scope, name);
    if (t)
      return t;
    scope = scope.parent
  }
...
function resolveReference(x, isModule) {
  if (x instanceof TReference) {
    ...
    if (x.resolving)
      throw new TypeError("Cyclic reference involving " + x);
    x.resolving = true;
    const base = resolveReference(x.base, true);
    ...
    for (let i = 1; i < x.names.length; i++) {
      const prty = t.properties.get(x.names[i]);
      const module = t.modules.get(x.names[i]);
...
function demodule(t) {
  t.modules.forEach(function (name, ref) {
    const module = resolveToObject(ref);
    if (t.properties.has(name)) {
      const typ = t.properties.get(name).type;
...
function substType(type, tenv) {
  if (type instanceof TObject) {
    ...
    t.properties = type.properties.mapv(e => substProperty(e, tenv));
    t.calls = type.calls.map(c => substCall(c, tenv));
    t.brand = type.brand;
    return t;
  }
  else if (type instanceof TGeneric) {
    return new TGeneric(substType(type.base, tenv), type.args.map(a => substType(a, tenv)))
  }
...
function inheritObject(obj) {
  ...
  obj.supers.forEach(function (sup) {
    ...
    if (sup instanceof TGeneric) {
      const base = resolveToObject(sup.base);
      inheritObject(base);
...
function outputTypeDef(td) {
  ...
  return {
    typeParameters: td.typeParameters.map(function (tp) {
      return tp.name
    }),
...
function findReferencedFiles(text) {
  const regex = /^\s*\/\/\/\s*<reference\s+path\s*=\s*["']([^"']*)["']\s*\/>/mig;
  let match;
  const matches = [];
...
function includeReferencedFiles(inputs) {
  const visited_files = Object.create(null);
  const worklist = [];
  inputs.forEach(function (input) {
...
function main() {
  ...
  program.commandHelp = function () {
    return "  Convert *.d.ts files to TypeScript Declaration Core\n"
  };
  ...
  const inputs = program.args.map(function (file) {
    return {
      file: file,
