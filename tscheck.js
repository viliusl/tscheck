...
function getArgumentWithExtension(ext) {
  if (program.args.length === 1) {
    ...
    return program.args.find(function (x) {
      return x.endsWith('.' + ext)
    })
  }
}

let snapshotWasGenerated = false;

function generateSnapshot(jsfile, jsnapfile, callback) {
  const jsnap = require('jsnap');
  snapshotWasGenerated = true;
  const fd = fs.openSync(jsnapfile, 'w');
  const proc = jsnap({
    files: [jsfile],
    stdio: ['ignore', fd, 2],
...
function checkSnapshot(jsfile, jsnapfile, callback) {
  if (fs.existsSync(jsnapfile)) {
    const js_stat = fs.statSync(jsfile);
    const jsnap_stat = fs.statSync(jsnapfile);
...
const LIB_ORIGIN = ">lib.d.ts"; // pad origin with ">" to ensure it does not collide with user input


let snapshot, typeDecl, sourceFileAst;

function initialize() {
  let sourceFile = getArgumentWithExtension('js');
  if (!sourceFile) {
  ...
  let typeDeclFile = getArgumentWithExtension('d.ts');
  if (!typeDeclFile) {
  ...
  const jsnapExt = program.runtime === 'browser' ? 'jsnap' : 'jsnap_node';
  let snapshotFile = getArgumentWithExtension(jsnapExt);
  if (!snapshotFile) {
  ...
  function loadInputs(snapshotFile) {
    ...
    // Load TypeScript
    const typeDeclText = fs.readFileSync(typeDeclFile, 'utf8');

    const libFile = __dirname + "/lib/lib.d.ts";
    const libFileText = fs.readFileSync(libFile, 'utf8');
    ...
    // Load source code
    const sourceFileText = fs.readFileSync(sourceFile, 'utf8');
    try {
...
const unique_error_ids = new Map;
...
function findPrty(obj, name) {
  while (obj) {
    const prty = findPrtyDirect(obj, name);
    if (prty)
      return prty;
  ...
  switch (type.type) {
    case 'object':
      ...
      for (let k in type.properties) {
        const typePrty = type.properties[k];
...
function substCall(call, tenv) {
  const typeParamSubst = substTypeParameters(call.typeParameters, tenv);
  const typeParams = typeParamSubst.typeParams;
...
const canonical_cache = Object.create(null);
let canonical_next_number = 1;

function canonicalizeKey(key) {
  let value = canonical_cache[key];
  if (!value) {
...
function indexProperties(obj) {
  ...
  if (!obj.prototype)
    return;
  const parent = lookupObject(obj.prototype.key);
...
function lookupPath(path, e) {
  ...
  for (let i = 0; i < toks.length; i++) {
    ...
    const obj = lookupObject(value.key);
    let prty = obj.propertyMap.get(tok);
...
function determineEnums() {
  for (let qname in typeDecl.enums) {
    const paths = typeDecl.enums[qname];
    const values = paths.map(a => lookupPath(a, function (path) {
...
const native2callsigs = Object.create(null);
...
function markNatives() {
  ...
  function visit(value, type) {
    ...
    if (type.type === 'reference') {
      const h = canonicalizeType(type) + "~" + value.key;
      if (h in visited)
        return;
      visited[h] = true;
    ...
    if (obj.function && obj.function.type === 'native') {
      let list = native2callsigs[obj.function.id];
      if (!list) {
...
const object2brands = new Map;
const valid_brands = Object.create(null);
...
function markBrands() {
  ...
  function visit(type) {
    switch (type.type) {
      ...
      case 'object':
        if (type.brand) {
          let obj = lookupPath(type.brand + '.prototype', function () {
            return null
          });
...
function treatAsOptionalMember(type, prtyName) {
  const prty = type.properties[prtyName];
  const t = prty.type;
  if (prty.optional)
    return true;
...
const tpath2values = new Map;
const native_tpaths = new Map;
const assumptions = {};
  ...
  switch (type.type) {
    case 'object':
      ...
      if (must(typeof value === 'object')) {
        ...
        for (let k in type.properties) {
          const typePrty = type.properties[k];
          const isUserPrty = typePrty.meta.origin != LIB_ORIGIN;
          const isUserPath = userPath || isUserPrty;
...
function hasBrand(value, brand) {
  let ctor = lookupPath(brand, function () {
    return null
  });
  if (!ctor || typeof ctor !== 'object')
    return null;
  let proto = lookupObject(ctor.key).propertyMap.get('prototype');
...
function checkCallSignature(call, receiverKey, functionKey, path) {
  ...
  const functionObj = lookupObject(functionKey);
  ...
  const ok = analyzer.checkSignature(call, functionKey, receiverKey, path);
...
const SkipFunctionPrtys = ['name', 'length', 'arguments', 'caller', 'callee', 'prototype'];

function skipPrty(obj, name) {
  if (obj.function) {
    if (SkipFunctionPrtys.some(name))
      return true; // don't suggest built-in properties
    const funProto = lookupObject(FunctionPrototype.key);
...
function findSuggestions() {
  tpath2values.forEach(function (tpath, values) {
    ...
    names.forEach(function (name, count) {
      const alwaysPresent = (count === values.length);
      const optStr = alwaysPresent ? '' : ' (optional)';
  ...
  switch (type.type) {
    ...
    case 'reference':
      ...
      if (!coverage.reachable[type.name]) {
        ...
        const t = typeDecl.env[type.name];
        if (t) {
...
function formatTypeCall(call) {
  const newstr = call.new ? 'new' : '';
  const tparams = call.typeParameters.length === 0 ? '' : ('<' + call.typeParameters.map(formatTypeParameter).join(',') + '>');
  const vargStr = call.variadic ? ', ...' : '';
  ...
  if (typeof value === 'object' && value !== null) {
    switch (value.key) {
      ...
      default:
        ...
        const fn = obj.function ? 'Function ' : '';
        return fn + '{ ' + obj.properties.map(function (prty) {
          return prty.name + ': ' + formatValue(prty.value, depth - 1)
        }).join(', ') + ' }'
    }
  } else {
...
HashSet.prototype.clone = function () {
  const r = new this.constructor();// Object.create(Object.getPrototypeOf(this))
  for (let k in this) {
    if (this.hasOwnProperty(k)) {
      r[k] = this[k]
    }
...
function getFunction(id) {
  let f = sourceFileAst.$id2function[id];
  if (!f) {
...
function Analyzer() {
  ...
  // 		UNIFICATION		//
  //////////////////////////
  let unode_current_function = null;
  ...
  UNode.prototype = {
    ...
    get properties() {
      return this.rep()._properties
    },
    get primitives() {
      return this.rep()._primitives
    },
    get functions() {
      return this.rep()._functions
    },
    get call_sigs() {
      return this.rep()._call_sigs
    },
    get resolved_refs() {
      return this.rep()._resolved_refs
    },
    get prototypes() {
      return this.rep()._prototypes
    },
    get isAny() {
      return this.rep()._level === LEVEL_ANY
    },
    get isObject() {
      return this.rep()._isObject
    },
    set isObject(x) {
      this.rep()._isObject = x
    },
    get level() {
      return this.rep()._level
    },
    set level(x) {
      this.rep()._level = x
    },
    get isFree() {
      return this.rep()._isFree
    },
    set isFree(x) {
      return this.rep()._isFree = x
    },
    get brands() {
      return this.rep()._brands
    }
  };
  ...
  UNode.prototype.getPrty = function (name) {
    const r = this.rep();
    const wasEmpty = r.properties.size === 0;
    switch (r._level) {
      case LEVEL_BOT:
        break;
    ...
    if (wasEmpty) {
      r.primitives.forEach(function (t) {
        const coerced = primitiveToObjectType(t);
        if (coerced) {
          r.addType(coerced)
        }
      })
  ...
  UNode.prototype.clone = function () {
    ...
    if (r.global) {
      ...
      const target = r.clone_target = new UNode;
      ...
      target._prototypes = r._prototypes.map(function (x) {
        return x.clone()
      });
  ...
  function primitiveToObjectType(t) {
    const typ = t.type === 'value' ? (typeof t.value) : t.type;
    switch (typ) {
      case 'number':
        return {type: 'reference', name: 'Number', typeArguments: []};
      case 'string':
  ...
  UNode.prototype.addType = function (t) {
    ...
    switch (t.type) {
      ...
      case 'object':
        this.isObject = true;
        for (let k in t.properties) {
          const name = this.translateName(k);
          ...
          if (dst) {
            if (dst.type === 'node') {
              ...
              if (t.properties[k].type === 'node') {
                const targetNode = t.properties[k].node;
        ...
        if (t.brand) {
          let obj = lookupPath(t.brand + '.prototype', function () {
            return null
          });
          if (obj && typeof obj === 'object') {
      ...
      case 'string-const':
        ...
        break;
      case 'enum':
        const vals = enum_values.get(t.name);
        if (vals.length === 0) {
          this.makeAny();
        } else {
  ...
  function unifyNow(n1, n2) {
    ...
    if (n2.rank > n1.rank) {
      const z = n1;
      n1 = n2;
      n2 = z; // swap n1/n2 so n1 has the highest rank
    }
    if (n1.rank === n2.rank) {
      n1.rank++
    }
  ...
  FunctionNode.prototype.clone = function () {
    const fnode = Object.create(FunctionNode.prototype);
    fnode.arguments = this.arguments.clone();
    fnode.return = this.return.clone();
    fnode.this = this.this.clone();
    fnode.self = this.self.clone();
    fnode.calls = this.calls.map(function (x) {
      return x.clone()
    });
    fnode.inherits = this.inherits.map(function (x) {
      return x.clone()
    });
    fnode.dynamic_accesses = this.dynamic_accesses.map(function (x) {
      return x.clone()
    });
  ...
  CallNode.prototype.clone = function () {
    const call = Object.create(CallNode.prototype);
  ...
  const main_function = new FunctionNode;
  ...
  //			HEAP -> U-NODES 		//
  //////////////////////////////////////
  const object2node = Object.create(null);
  ...
  function buildHeap() {
    snapshot.heap.forEach(function (obj, i) {
      if (!obj)
        return;
      const n = object2node[i] = new UNode;
      n.isObject = true;
      n.global = true;
      if (obj.function) {
  ...
  snapshot.heap.forEach(function (obj, i) {
    ...
    if (obj.function && obj.function.type === 'native' & !special_natives.hasOwnProperty(obj.function.id)) {
      const sigs = getCallSigsForNative(obj.function.id);
      if (sigs.length === 1) {
  ...
  // 		AST -> U-NODES		//
  //////////////////////////////
  const function2fnode = Object.create(null);

  function getPristineFunctionNode(fun) {
    let fnode = function2fnode[fun.$id];
    if (!fnode) {
  ...
  function makeFunction(fun) { // ast-node -> FunctionNode
    const ast2node = Object.create(null);

    function getNode(x) {
      if (x instanceof UNode)
        return x.rep();
      const n = ast2node[x.$id];
      if (n) {
        return n.rep()
      } else {
    ...
    function getVar(id) {
      let scope = getEnclosingScope(id);
    ...
    const NULL = 'NULL'; // result of expression is null or undefined
    const NOT_NULL = 'NOT_NULL';
    const VOID = 'VOID'; // result of expression is discarded or immediately coerced to a boolean
    const NOT_VOID = 'NOT_VOID';
      ...
      switch (node.type) {
        ...
        case 'TryStatement':
          ...
          if (node.handler) {
            ...
            const catchEnv = getNode(node.handler);
        ...
        case 'VariableDeclaration':
          node.declarations.forEach(function (d) {
            ...
            if (d.init) {
              const p = visitExp(d.init, NOT_VOID);
              if (p === NULL) {
      ...
      switch (node.type) {
        ...
        case 'ObjectExpression':
          ...
          node.properties.forEach(function (p) {
            ...
            const name = p.key.type === 'Literal' ? String(p.key.value) : p.key.name;
            switch (p.kind) {
              case 'init':
        ...
        case 'AssignmentExpression':
          if (node.operator === '=') {
            ...
            const r = visitExp(node.right, NOT_VOID);
            if (r === NULL) {
  ...
  const function2shared = Object.create(null);

  function getSharedFunctionNode(fun, context) {
    complete();
    const h = fun.$id + '~' + context;
    let fnode = function2shared[h];
    if (fnode)
      return fnode;
    setCurrentFunction(null);
    ...
    while (changed) {
      ...
      entry_calls.forEach(function (ecall) {
        ecall.self.functions.forEach(function (fun) {
          if (fun.type !== 'user')
            return;
          const fnode = getSharedFunctionNode(getFunction(fun.id), fun.context);
          ...
          const demand_cache = Object.create(null);
          const resultNode = ecall.sig.new ? fnode.this : fnode.return;
          ...
          function demandNodeVsType(node, type) {
            switch (type.type) {
              ...
              case 'opaque-type':
                break;
              case 'reference':
                const h = node.id + '~' + canonicalizeType(type);
          ...
          function demandTypeVsType(t, type) {
            switch (type.type) {
              ...
              case 'reference':
                ...
                const h = canonicalizeType(t) + '~~' + canonicalizeType(type);
                if (h in demand_cache)
                  return;
                demand_cache[h] = true;
      ...
      main_function.calls.forEach(function (call) {
        ...
        callee.call_sigs.forEach(function (sig) {
          ...
          const tcheck = makeTypeChecker(sig.typeParameters);
          let applicable = true;
          sig.parameters.forEach(function (param, i) {
            ...
            if (!call.arguments.properties.has(i)) { // note: use == when comparing i to a number
              const optional = param.optional || (sig.variadic && i == sig.parameters.length - 1); // last param of variadic functions is optional
              applicable &= optional;
              return
            }
          ...
          complete();
          const ret = sig.new ? call.this : call.return;
          ...
          tcheck.getCallSigMatches().forEach(function (cmatch) {
            ...
            let sig = substCall(cmatch.callsig, tenv);
  ...
  function widenPolymorphicCallSig(sig) {
    ...
    return {
      ...
      typeParameters: [],
      parameters: sig.parameters.map(function (x) {
        return substParameter(x, tenv)
      }),
  ...
  function sig2path(sig) {
    let tpStr = '';
    if (sig.typeParameters.length > 0) {
      tpStr = '<' + sig.typeParameters.map(function (tp) {
        return tp.name
      }).join(',') + '>'
    }
    return tpStr + '(' + sig.parameters.map(function (x) {
      return formatType(x.type)
    }).join(',') + ')'
  }
  ...
  function makeTypeChecker(freeTypeVars) {
    ...
    const node_prty_compatible = Object.create(null);
    ...
    function isNodeCompatibleWithProperty(node, k, prty, path, receiver) {
      ...
      const name = node.translateName(k);
      const dst = node.properties.get(name);
      if (dst) {
        return isPrtyCompatibleWithType(dst, prty.type, path, receiver)
      } else {
        let protoWithPrty = node.prototypes.filter(function (proto) {
          return proto.properties.has(name)
        });
        protoWithPrty = _.uniq(protoWithPrty, proto => proto.rep().id);
        if (protoWithPrty.length === 0) {
          ...
          const h = node.id + '~' + k + '~' + prty.optional + '.' + canonicalizeType(prty.type);
          if (h in node_prty_compatible)
            return node_prty_compatible[h];
          node_prty_compatible[h] = false;
          let ok = node.prototypes.some(function (proto) {
    ...
    const node_type_compatible = Object.create(null);
    ...
    function isNodeCompatibleWithType(node, type, path, receiver, continueOnError) {
      ...
      const h = canonicalizeType(type) + "~" + node.id; // TODO: include receiver??
      ...
      switch (type.type) {
        case 'object':
          ...
          if (type.brand && isValidBrand(type.brand)) {
            let hasBrand = node.brands.length > 0;
            ok = node.brands.includes(type.brand);
            if (!ok) {
              ok = node.prototypes.some(function (proto) {
                return hasBrand |= proto.brands.includes(type.brand)
              })
            }
            if (!ok) {
              if (hasBrand) {
          ...
          return ok;
        case 'enum':
          const enum_vals = enum_values.get(type.name);
    ...
    function isTypeCompatibleWithTypeX(t1, t2, path, continueOnError) {
      ...
      if (t1.type === 'reference' && t2.type === 'reference' && t1.name === t2.name) {
        ...
        var nok = t1.typeArguments.some((p1, i) => !isTypeCompatibleWithType(p1, t2.typeArguments[i], null));
        if (nok) {
      ...
      if (t2.type === 'object' && (t1.type !== 'object' && t1.type !== 'reference')) {
        ...
        const t1o = coerceTypeToObject(t1);
    ...
    function isCallCompatibleWithCall(incall, outcall) {
      ...
      for (let i = 0; i < incall.parameters.length; ++i) {
        const iparm = incall.parameters[i];
        if (i < outcall.parameters.length) {
          const oparm = outcall.parameters[i];
    ...
    return {
      ...
      getBoundTypeParams: function () {
        return bound_type_params
      },
      getBoundNodes: function () {
        return bound_nodes
      },
      getCallSigMatches: function () {
        return callSigMatches
      },
    }
  }
  ...
  function instantiateOpaqueSignature(sig) {
    ...
    const tenv = new Map;
    sig.typeParameters.forEach(function (tp) {
      const tnode = new UNode;
      tenv.put(tp.name, {type: 'opaque-type', name: tp.name});
      if (tp.constraint) {
        const constraint = substType(tp.constraint, tenv);
  ...
  this.checkSignature = function (sig, function_key, receiver_key, path) {
    setCurrentFunction(main_function);
    const call = new CallNode({new: sig.new, context: '0'});

    const originalSig = sig;
    sig = instantiateOpaqueSignature(sig);
    // TODO: varargs
    ...
    const resultNode = sig.new ? call.this : call.return;
    const resultType = sig.returnType;

    let tcheck = makeTypeChecker();
    ...
    let continueOnError = sig.new;
    let ok = tcheck.isNodeCompatibleWithType(resultNode, resultType, path + sig2path(originalSig), null, continueOnError);
    ...
    const toCheck = [];
    tcheck.getCallSigMatches().forEach(function (cmatch) {
      const ok = cmatch.node.call_sigs.some(function (s) {
        return tcheck.isCallCompatibleWithCall(s, cmatch.callsig)
      });
      if (ok)
        return;

      const sig = instantiateOpaqueSignature(cmatch.callsig);
      resolveEntryCallLater(new EntryCallNode(cmatch.node, cmatch.receiver, sig));
      toCheck.push({
        new: sig.new,
        node: cmatch.node,
    ...
    toCheck.forEach(function (chk) {
      ...
      chk.node.rep().functions.forEach(function (fun) {
        if (fun.type !== 'user')
          return true;
        const fnode = getSharedFunctionNode(getFunction(fun.id), fun.context);
        const resultNode = chk.new ? fnode.this : fnode.return;
  ...
  function Graphviz() {
    ...
    const seen_nodes = Object.create(null);
    ...
    const node2live = Object.create(null);
