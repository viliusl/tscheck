...
TypeScript.parse = function(text) {
  const fancyText = TypeScript.SimpleText.fromString(text);

  const allowSemicolonInsertion = true;
  const allowModuleKeyword = true;
  const options = new TypeScript.ParseOptions(
    TypeScript.LanguageVersion.EcmaScript5,
    allowSemicolonInsertion,
    allowModuleKeyword);

  const isDecl = true;
  const syntaxTree = TypeScript.Parser.parse('', fancyText, isDecl, options);

  const lineMap = TypeScript.LineMap.fromString(text);
  const compilationSettings = new TypeScript.CompilationSettings();
  const visitor = new TypeScript.SyntaxTreeToAstVisitor('', lineMap, compilationSettings);
  const ast = syntaxTree.sourceUnit().accept(visitor);
